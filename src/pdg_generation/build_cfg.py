...
def do_while_cf(node):
    ...
    # extra_comment_node(node, 2)  # No more comment dependency
    ...
    for child in ast_nodes.children:
        if child.name in _node.EPSILON or child.name in _node.UNSTRUCTURED:
            ...
            for grandchild in child.children:
                ...
                # I do not see why a non-statement child should have an epsilon CF to
                # its statement grandchild, and not a statement dependency...
                """
                if not isinstance(grandchild, _node.Statement):
                    link_expression(node=grandchild, node_parent=child)
                else:
