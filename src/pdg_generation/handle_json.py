...
def create_node(dico, node_body, parent_node, cond=False):
    ...
    if dico is None:  # Not a Node, but needed a construct to store, e.g., [, a] = array
        node = Node(name='None', parent=parent_node)
        ...
        if dico['type'] in STATEMENTS:
            node = Statement(name=dico['type'], parent=parent_node)
        elif dico['type'] == 'Literal' or dico['type'] == 'ArrayExpression'\
                or dico['type'] == 'ObjectExpression':
            node = LiteralArrayObject(name=dico['type'], parent=parent_node)
        elif dico['type'] != 'Identifier':
            node = Node(name=dico['type'], parent=parent_node)
        else:
            node = Identifier(name=dico['type'], parent=parent_node)
        parent_node.set_child(node)
        node.set_body(node_body)
        if cond:
        ...
        for child in ast_nodes.children:
            ...
            if child.body_list:
                ...
                if not dico2:  # Case [, a] = array -> [None, {stuff about a}] (None and not {})
                    dico2 = None  # Not sure if it could not be legitimate sometimes, TODO
